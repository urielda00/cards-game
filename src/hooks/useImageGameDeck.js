...
import { useImageCards } from '../context/ImageCardContext';
...
export const useImageGameDeck = (filteredCards) => {
	const { dispatch, listId } = useImageCards();
	...
	const startNewSession = useCallback(() => {
		if (filteredCards && filteredCards.length > 0) {
			...
			filteredCards.forEach((card) => {
				const weight = card.bucket ? weights[card.bucket - 1] || 1 : 1;
	...
	const updateCard = async (cardId, updates) => {
		try {
			const response = await fetch(`${import.meta.env.VITE_API_URL}/api/image-cards/${cardId}`, {
				method: 'PATCH',
	...
	const handleAnswer = useCallback(
		(isSuccess) => {
			...
			const currentBucket = currentCard.bucket || 1;
			if (isSuccess) {
	...
	const handleDelete = async () => {
		try {
			await fetch(`${import.meta.env.VITE_API_URL}/api/image-cards/${currentCard.id}`, { method: 'DELETE' });
			toast.success(`"${currentCard.back}" נמחקה.`);
