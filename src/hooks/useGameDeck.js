...
export const useGameDeck = (filteredCards) => {
    ...
    const cardIds = useMemo(() => 
        filteredCards.map(c => c.id).sort().join(','), 
    [filteredCards]);
    ...
    const startNewSession = useCallback(() => {
        if (filteredCards && filteredCards.length > 0) {
            ...
            filteredCards.forEach(card => {
                ...
                for (let i = 0; i < weight; i++) { weightedDeck.push(card); }
            });
    ...
    const handleSessionComplete = useCallback(async () => {
        ...
        try {
            await fetch(`http://localhost:3001/api/lists/${listId}/complete`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ completed: true }),
            });
        } catch (error) { console.error("Failed to mark list as complete:", error); }
    }, [listId]);
    ...
    const updateCard = async (cardId, updates) => {
        try {
            const response = await fetch(`http://localhost:3001/api/words/${cardId}`, {
    ...
    const handleAnswer = useCallback((isSuccess) => {
    ...
    const toggleStarred = async () => {
        ...
        if (updatedCard.starred) { toast.success('נוספה למועדפים! ⭐'); } 
        else { toast('הוסרה מהמועדפים.'); }
    };

    const handleDelete = async () => {
        try {
            await fetch(`http://localhost:3001/api/words/${currentCard.id}`, { method: 'DELETE' });
