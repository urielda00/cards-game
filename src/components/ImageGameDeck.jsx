...
function ImageGameDeck({ filteredCards }) {
    ...
    if (isSessionComplete) {
        return (
            <SessionCompleteWrapper>
                ...
                <StartNewSessionButton to="/image-lists" onClick={startNewSession}>
                    חזרה לרשימות
    ...
    if (!currentCard) {
        return <NoCardsMessage><h2>אין כרטיסיות ברשימה זו.</h2></NoCardsMessage>;
    }
    ...
    return (
        <>
            ...
            >
                <p>האם למחוק את הכרטיסייה: <br /><strong>"{currentCard?.back}"</strong>?</p>
            </ConfirmModal>
            <GameDeckWrapper>
                <CardAnimationWrapper className={exitDirection ? `exit-${exitDirection}` : ''}>
                    <FlashcardWithControls>
                        ...
                        <BucketDisplay>רמה: {currentCard.bucket || 1}</BucketDisplay>
                    </FlashcardWithControls>
