...
const loadDataIntoCache = async () => {
  try {
    ...
    dataCache.imageCards = JSON.parse(await fs.readFile(path.join(dataPath, 'imageCards.json'), 'utf8'));
    dataCache.imageLists = JSON.parse(await fs.readFile(path.join(dataPath, 'imageLists.json'), 'utf8'));
...
app.post('/api/lists', async (req, res) => {
  ...
  } catch (err) { res.status(500).send({ message: 'Error saving new list' }); }
});
app.patch('/api/lists/:id', async (req, res) => {
  ...
  } catch (err) { res.status(500).send({ message: 'Error saving list' }); }
});
app.delete('/api/lists/:id', async (req, res) => {
    const { id } = req.params;
    const initialListLength = dataCache.lists.length;
    dataCache.lists = dataCache.lists.filter(l => l.id !== id);
    if (dataCache.lists.length === initialListLength) return res.status(404).send({ message: 'List not found' });
    ...
    try {
        ...
        res.status(204).send();
    } catch (err) { res.status(500).send({ message: 'Error deleting list data' }); }
});
app.patch('/api/lists/:id/complete', async (req, res) => {
    ...
    const listIndex = dataCache.lists.findIndex(l => l.id === id);
    if (listIndex === -1) return res.status(404).send({ message: 'List not found' });
    if (typeof completed !== 'boolean') return res.status(400).send({ message: 'Completed status must be a boolean' });
    dataCache.lists[listIndex].completed = completed;
    try {
        await fs.writeFile(path.join(dataPath, 'lists.json'), JSON.stringify(dataCache.lists, null, 2));
        res.status(200).send(dataCache.lists[listIndex]);
    } catch (err) { res.status(500).send({ message: 'Error saving list completion status' }); }
});
...
app.post('/api/lists/:listId/words', async (req, res) => {
  ...
  if (!dataCache.lists.find(l => l.id === listId)) return res.status(404).send({ message: 'List not found' });
  const newWord = { id: `w${Date.now()}`, type: 'word', front, back, bucket: 1, starred: false, listId: listId, };
  dataCache.words.push(newWord);
  try {
    await fs.writeFile(path.join(dataPath, 'words.json'), JSON.stringify(dataCache.words, null, 2));
    res.status(201).send(newWord);
  } catch (err) { res.status(500).send({ message: 'Error saving data' }); }
});
...
app.delete('/api/words/:id', async (req, res) => {
    ...
    if (dataCache.words.length === initialLength) return res.status(404).send({ message: 'Card not found' });
    try {
        await fs.writeFile(path.join(dataPath, 'words.json'), JSON.stringify(dataCache.words, null, 2));
        res.status(204).send();
    } catch (err) { res.status(500).send({ message: 'Error saving data' }); }
...
app.post('/api/image-lists', async (req, res) => {
  ...
  try {
    await fs.writeFile(path.join(dataPath, 'imageLists.json'), JSON.stringify(dataCache.imageLists, null, 2));
    res.status(201).send(newList);
  } catch (err) { res.status(500).send({ message: 'Error saving new image list' }); }
});
app.delete('/api/image-lists/:id', async (req, res) => {
    ...
    if (dataCache.imageLists.length === initialLength) return res.status(404).send({ message: 'Image list not found' });
    dataCache.imageCards = dataCache.imageCards.filter(c => c.listId !== id);
    try {
        await fs.writeFile(path.join(dataPath, 'imageLists.json'), JSON.stringify(dataCache.imageLists, null, 2));
        await fs.writeFile(path.join(dataPath, 'imageCards.json'), JSON.stringify(dataCache.imageCards, null, 2));
        res.status(204).send();
    } catch (err) { res.status(500).send({ message: 'Error deleting image list data' }); }
});
...
app.delete('/api/image-cards/:id', async (req, res) => {
    ...
    if (dataCache.imageCards.length === initialLength) return res.status(404).send({ message: 'Image card not found' });
    try {
        await fs.writeFile(path.join(dataPath, 'imageCards.json'), JSON.stringify(dataCache.imageCards, null, 2));
        res.status(204).send();
    } catch (err) { res.status(500).send({ message: 'Error saving data' }); }
